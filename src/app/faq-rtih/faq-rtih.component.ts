...
type Faq = { q: string; a: string; expanded?: boolean };

@Component({
  selector: 'app-faq-rtih',
  standalone: true,
  imports: [CommonModule, MatExpansionModule, MatButtonModule],
  templateUrl: './faq-rtih.component.html',
  styleUrls: ['./faq-rtih.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FaqRtihComponent {
  readonly PREVIEW_COUNT = 4;
  showAll = false;
  ...
  faqs: Faq[] = [
    {
      ...
      to promote innovation, entrepreneurship, and industry–academia collaboration. 
      It acts as a central point of support for startups and innovators.`,
      expanded: true
    },
    {
      ...
      and create a vibrant startup ecosystem in Andhra Pradesh, with a vision to enable “One Family One Entrepreneur” 
    ...
    },
    {
      ...
      conference halls, and curated startup programs—may have pricing based on usage.`,
      expanded: false
    },
    {
  ...
  get visibleFaqs(): Faq[] {
    return this.showAll ? this.faqs : this.faqs.slice(0, this.PREVIEW_COUNT);
  }

  toggleMore(): void {
    if (this.showAll) {
      // collapse any panels beyond preview when hiding
      for (let i = this.PREVIEW_COUNT; i < this.faqs.length; i++) {
        this.faqs[i].expanded = false;
      }
      // optional: scroll back to top of FAQ section
      document.querySelector('section.faq')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
